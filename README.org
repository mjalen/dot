...
parted /dev/<hard-drive> -- mkpart ESP 1MB 512MB
parted /dev/<hard-drive> -- mkpart primary 512MB 100%
parted /dev/<hard-drive> -- set 1 esp on
#+end_src
...
#+begin_src bash
cryptsetup luksFormat /dev/<hard-drive>1 # answer prompts and provide password
cryptsetup luksOpen /dev/<hard-drive>1 crypted # open hard drive as /dev/mapper/crypted
#+end_src
...
#+begin_src bash
mkfs.fat -F 32 -n boot /dev/<hard-drive>2
...
mount /dev/<hard-drive>1 /mnt/boot
#+end_src
...
In the hardware file ~/mnt/persist/dot/systems/valhalla/default.nix~, update the PARTUUIDs and UUIDs. As a tip, paste the ~lsblk~ output into VIM by doing an execute and read: ~:r !lsblk ...~. This will save on copy/paste headaches with multiple terminals. Remember to remove that output after you have finished collecting the proper UUIDs.

#+begin_src nix 
# line 8 ...
valhalla.hardware = {
    enabled = true;
    encryptedPARTUUID = "..."; # /dev/<hard-drive>2 -> PARTUUID 
    bootUUID = "..."; # /dev/<hard-drive>1 -> UUID
...
#+begin_src bash
nixos-install --impure --flake .#robin
#+end_src
